...
const DropdownMenuItem = forwardRef<HTMLDivElement, Radix.DropdownMenuItemProps>(
  ({ className, ...rest }, ref) => (
    <Radix.Item
      ref={ref}
      className={cn(
        "relative flex cursor-pointer select-none items-center gap-2 rounded-md px-2.5 py-1.5 text-sm text-text-muted outline-none",
...
const DropdownMenuLabel = forwardRef<HTMLDivElement, Radix.DropdownMenuLabelProps>(
  ({ className, ...rest }, ref) => (
    <Radix.Label
      ref={ref}
      className={cn(
        "px-2.5 py-1.5 text-sm font-medium uppercase tracking-wider text-text-faint",
        className,
      )}
      {...rest}
    />
