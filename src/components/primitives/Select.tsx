  ...
  ({ className, size = "md", children, ...rest }, ref) => (
    <Radix.Trigger
      ...
      className={cn(
        ...
        size === "sm" ? "h-7 text-xs" : "h-8 text-xs-plus",
        className,
      )}
      {...rest}
    >
...
const SelectItem = forwardRef<HTMLDivElement, Radix.SelectItemProps>(
  ({ className, children, ...rest }, ref) => (
    <Radix.Item
      ref={ref}
      className={cn(
        "relative flex cursor-pointer select-none items-center rounded-md py-1.5 pl-7 pr-2 text-xs text-text-muted outline-none",
