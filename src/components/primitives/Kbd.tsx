...
export function Kbd({ className, children, ...rest }: HTMLAttributes<HTMLElement>) {
  return (
    <kbd
      className={cn(
        "inline-flex items-center justify-center rounded-md border border-border bg-accent-soft px-1 h-kbd min-w-kbd text-xs font-mono font-medium text-charcoal leading-none tracking-tight",
        className,
      )}
      {...rest}
    >
