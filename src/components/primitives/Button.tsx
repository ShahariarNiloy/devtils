...
const variants: Record<Variant, string> = {
  ...
  secondary:
    "bg-transparent text-text border border-brand hover:bg-surface-soft active:scale-[0.98] disabled:opacity-50",
  ghost:
...
const sizes: Record<Size, string> = {
  sm: "h-8 px-3 text-xs-plus gap-1.5 rounded-lg",
  ...
  (
    ...
    return (
      <Comp
        ...
        type={inferredType}
        className={cn(
          "inline-flex items-center justify-center whitespace-nowrap font-sans font-semibold transition-[background,color,border-color,transform,box-shadow] duration-150 ease-out disabled:pointer-events-none select-none",
          variants[variant],
          sizes[size],
          className,
        )}
