 ...
 */
export function TierChips({ value, onChange, counts, totalCount }: TierChipsProps) {
  ...
  const allChips: { tier: ToolTier | null; label: string; count: number; dot?: string }[] = [
    { tier: null, label: "All", count: totalCount },
    { tier: "free", label: TIER_LABEL.free, count: counts.free, dot: TIER_DOT.free },
    { tier: "pro", label: TIER_LABEL.pro, count: counts.pro, dot: TIER_DOT.pro },
  ...
  return (
    <div role="group" aria-label="Filter by tier" className="flex items-center gap-1.5 shrink-0">
      {chips.map((c) => {
